...
import pandas as pd
import datetime
from assisfunc.newjobtask.assisfuncfjob import data_to_resp
...
for i, iclient in enumerate(all_clients_id, 1):
    ...
    date_start = addi_data.dataLiability.dateStart[iclient]  # список дат начала 
обязательств
    date_end = addi_data.dataLiability.dateEnd[iclient]  # список дат окончания 
обязательств
    sum_delay = 0 # sum(addi_data.dataLiability.sumDelay[iclient])  # сумма 
просрочки
    ...
    statusCHIP = addi_data.client[iclient]['citizenship']  # definition of 
citizenship
    if statusCHIP != 'RU':
    ...
    dateREGIST = addi_data.source.dateRegistration[iclient]  # definition of 
activity
    current_date = datetime.datetime.now().strftime('%d.%m.%Y')
    date_reg = datetime.datetime(int(dateREGIST[-4:]), int(dateREGIST[3:5]), 
int(dateREGIST[:2]))
